...
function renderStatic(){
 renderStaticScaffold();
 const construction=LATEST.filter(r=>r.sector==='23'&&r.structural_readiness_score!=null).sort((a,b)=>b.structural_readiness_score-a.structural_readiness_score)[0];
 if(construction){$('#spotlightValue').textContent=`${construction.state_name} · ${construction.structural_readiness_score.toFixed(1)}`;$('#spotlightText').textContent=`Highest 2023 Structural Readiness Score in the Construction state cross-section; ${fmt(construction.estabs)} establishments and ${pct(construction.estabs_entry_rate)} entry rate.`;$('#spotlightSource').textContent='Derived from U.S. Census BDS 2023 · descriptive, not investment advice';}
 $('#sourceGrid').innerHTML=sources.map(s=>`<article class="source-card"><span class="status">${s.status}</span><h3>${s.name}</h3><p><b>${s.agency}</b><br>${s.desc}</p><a href="${s.url}" target="_blank" rel="noreferrer">Open official source ↗</a></article>`).join('');
...
function renderRanking(){const q=$('#rankSearch').value.toLowerCase().trim();let rows=LATEST.filter(r=>r.structural_readiness_score!=null).sort((a,b)=>b.structural_readiness_score-a.structural_readiness_score);if(q)rows=rows.filter(r=>(r.state_name+' '+r.sector_name).toLowerCase().includes(q));rows=rows.slice(0,40);$('#rankBody').innerHTML=rows.map((r,i)=>`<tr><td>${i+1}</td><td><b>${r.state_name}</b></td><td>${r.sector_name}</td><td><span class="score-chip">${r.structural_readiness_score.toFixed(1)}</span></td><td>${fmt(r.firms)}</td><td>${fmt(r.estabs)}</td><td>${pct(r.estabs_entry_rate)}</td><td>${pct(r.net_job_creation_rate)}</td></tr>`).join('')}
